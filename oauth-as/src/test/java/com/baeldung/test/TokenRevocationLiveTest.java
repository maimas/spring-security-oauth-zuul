...
public class TokenRevocationLiveTest {
    ...
    public void whenObtainingAccessToken_thenCorrect() {
        ...
        final Response resourceServerResponse = RestAssured.given().header("Authorization", "Bearer " + accessToken).get("http://localhost:8082/oauth-rs/foos/100");
    ...
    private Response obtainAccessToken(String clientId, String username, String password) {
        ...
        return RestAssured.given().auth().preemptive().basic(clientId, "secret").and().with().params(params).when().post("http://localhost:8081/oauth-as/oauth/token");
    ...
    private String obtainRefreshToken(String clientId, final String refreshToken) {
        ...
        final Response response = RestAssured.given().auth().preemptive().basic(clientId, "secret").and().with().params(params).when().post("http://localhost:8081/oauth-as/oauth/token");
    ...
    private void authorizeClient(String clientId) {
        ...
        final Response response = RestAssured.given().auth().preemptive().basic(clientId, "secret").and().with().params(params).when().post("http://localhost:8081/oauth-as/oauth/authorize");
    }
}